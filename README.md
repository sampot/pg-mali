# pg-mali

復古**二十四格燈圈小瑪莉**瀏覽器小品（機台感面板）：押注 → 跑燈 → 結算。純娛樂、無真實金錢／兌現。

- **音效：** Web Audio 即時合成（投幣／押注／跑燈／中獎），無商業機台取樣  
- **圖示：** 八種原創輕量 SVG 剪影  
- **中心區：** 待機流水燈、跑燈加速、中獎彩光  

也可當作 [Playgrounds（遊樂場）](https://play.samkuo.me/) 的 **SAM**（`index.html` 入口）。

## 一鍵開 SAM 小

**[一鍵開 SAM 小](https://play.samkuo.me/?open=sampot%2Fpg-mali&name=%E5%B0%8F%E7%91%AA%E8%8E%89)**

```
https://play.samkuo.me/?open=sampot/pg-mali&name=小瑪莉
```

同源會重用本機已匯入的沙盒；要強制新建可加 `&fresh=1`。

## 試玩（本機）

```bash
npx --yes serve .
# 或
python3 -m http.server 8080
```

瀏覽器需允許音訊（點一下頁面後音效才會出聲）。

## 操作

| 操作 | 說明 |
| --- | --- |
| **+50 幣** | 增加娛樂幣 |
| 點下方圖案 | 每點押 1 幣 |
| **開始** | 跑燈（聲畫同步）後結算 |
| **撤銷押注** | 退回本局未開跑的押注 |
| **音效開／關** | 靜音切換 |

二十四格燈圈（7×7 外圈邊框 + 5×5 中央計分區，沿邊跑燈）、九種圖案與賠率為本專案原創配置，僅向「小瑪莉機台」類型致敬，不還原任何特定機台。抽籤依權重控制，單一符號押注的期望報酬（RTP）約 0.97（玩家略佔優的約 3% 莊家優勢），純娛樂、非公平零和。

## 檔案

| 檔案 | 說明 |
| --- | --- |
| `index.html` | 結構 |
| `styles.css` | 機台感版面 |
| `app.js` | UI 與跑燈動畫 |
| `game.js` | 押注／賠率／路徑 |
| `icons.js` | 八種圖案輕量 SVG（原創剪影） |
| `audio.js` | Web Audio 合成音效 |
| `functions.js` | Playgrounds 可選 stub |

## License

MIT
